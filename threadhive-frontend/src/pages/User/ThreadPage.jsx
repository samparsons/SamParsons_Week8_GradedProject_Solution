...
export default function Thread() {
  ...
  useEffect(() => {
    if (threadId) {
      ...
      dispatch(fetchCommentsThunk(threadId));
    }
  ...
  const handlePostComment = async () => {
    ...
    const resultAction = await dispatch(
      postCommentThunk({ threadId, content: trimmedComment }),
    );

    if (postCommentThunk.fulfilled.match(resultAction)) {
      setCommentText("");
    }
  };
  ...
  return (
    <Container className="thread-container">
      ...
      <CommentForm
        ...
        disabled={commentsPosting || !commentText.trim()}
      />
